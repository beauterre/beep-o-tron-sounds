...
function createRecipe(mods,tones,musical)
{
		...
		if(Math.random()<0.6) // mostly
		{
			...
			if(musical || Math.random()<0.1)
			{
				...
				{	// create frequency envelope!
					recipe.o[recipe.o.length-1].fe=createRandomEnvelope();
				}else
				{// create volume envelope for oscillator
					recipe.o[recipe.o.length-1].ve=createRandomEnvelope();
				}
			}
		}else
		{
	...
	{		//if browser supports toFixed() method
		...
		{
			var o=recipe.o[i];
			o.f=o.f.toFixed(3);// no more than 3 significant numbers
			o.v=o.v.toFixed(3);// no more than 3 significant numbers
		}		
	}else{
		// make values neater
		for(i=0;i<recipe.o.length;i++)
	...
	recipe.k="beep"; // the o-tron is not strictly necessary.
	recipe.v=1.0;
	var total_connected=[];
	for(i=0;i<recipe.o.length;i++)total_connected[i]=0;
	
	for(i=0;i<recipe.o.length;i++)
	{
		if(recipe.o[i].c!=-1) total_connected[recipe.o[i].c]+=Math.abs(parseInt(recipe.o[i].v))+Math.abs(parseInt(recipe.o[i].f))+Math.abs(recipe.o[recipe.o[i].c].f);
	}
	for(i=0;i<recipe.o.length;i++)
	{
		if(total_connected!=0)
		{
			if(recipe.o[i].c==-1) recipe.o[i].v=(10+recipe.o[i].v)/(total_connected[i]+1000);
		}
	}
	return recipe;
}
function createRandomEnvelope(entries)
{
	var envelope=[];
	envelope.push({t:0,v:Math.floor(Math.random()*10)/10});
	envelope.push({t:1,v:Math.floor(Math.random()*10)/10});
	...
	envelope.sort(sortOnTime);
	// now filter out anything that has the same time as prev.
	var answer=[];
	for(i=0;i<envelope.length;i++)
	{
		if(answer.length==0 || envelope[i].t!=answer[answer.length-1].t)
		{
			answer.push(envelope[i]);
		}
	}
	return answer;	
}
