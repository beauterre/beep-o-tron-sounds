...
if(typeof(Storage) !== "undefined") {
	...
	}else
	{
		...
		if(typeof(saved.v)==="undefined")
		{
			...
			console.log("previous cookie not usefull, initialising for");
...
function exportTemplates()
{
	...
	var i;
	var nrofsnds=0;
	var exp=JSON.parse(JSON.stringify(saved));// copy for safety
	for(i=0;i<exp.cat.length;i++)
	{
		var s;
		for(s=0;s<exp.cat[i].snds.length;s++)
		{
			var snd=JSON.parse(exp.cat[i].snds[s].r);
			snd.v=1.0; // these should be added to the recipy!
			snd.k="beep";
			exp.cat[i].snds[s].r=snd;
			nrofsnds++;
		}
	}
	...
	console.log("----------------templates---------------------");	
	console.log("Total number of sounds in library: "+nrofsnds);	
}
...
function saveRecipe()
{
	...
	if(name!=null)
	{
		saved_array.push({name:name,r:JSON.stringify(recipe,stuntValues)});
		showSaved();
	}
}
function stuntValues(key, val)
{
    return val.toFixed ? Number(val.toFixed(6)) : val; // no more than 3 significant values.
}
